...
public class SmallMenuHandler {
    ...
    public SmallMenuHandler(XiWheelMenus plugin) {
        this.plugin = plugin;
    }
    ...
    public void openMenu(Player player, String fileName) {
        ...
        double angleStep = 360.0 / Math.min(itemCount, 8); // 360 degrees divided by min(itemCount, 8)
    ...
    public void closeMenu(Player player) {
        ...
        if (armorStands != null && !armorStands.isEmpty() && armorStandLocations != null && menuCenter != null) {
            ...
            // Create a list of current locations of the armor stands
            ...
            animateArmorStands(player, currentLocations, menuCenter, closeAnimationSteps, () -> {
    ...
    private void animateArmorStandsCommon(Player player, List<Location> fromLocations, List<Location> toLocations, int totalSteps, Runnable onComplete) {
        ...
        BukkitRunnable animationTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < Math.min(armorStands.size(), Math.min(fromLocations.size(), toLocations.size())); i++) {
